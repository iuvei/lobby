...
import {
  ...
  modifyPlanMultiple,
  initAdvancedPlanOne,
  initAdvancedPlanTwo, handlePlanChecked
...
class BetChase extends Component {
  ...
  render() {
    const { orderList, planList, handleInitPlan, handlePlanMultiple, issueList, winStop, setWinStop } = this.props
    let totalPrice = 0
    if (orderList.length <= 0) {
      return null
    } else {
      for (let order of orderList) {
        totalPrice = MathAdd(totalPrice, order.total)
      }
    }
        ...
        <div className="chase-table">
          <div className="ct-header">
            <div className="h-label">
              ...
              <div className="h-control" onClick={e => {setWinStop()}}>
                <i className={`iconfont ${winStop ? 'icon-checkbox-check' : 'icon-checkbox-uncheck'}`}/>
                <span>中奖后停止追号</span>
              </div>
            </div>
                ...
                planList.map((plan ,index) =>
                  <div className="trow" key={index}>
                    ...
                    <div className="ct-amount">
                      {plan.checked === false ? 0 : MathMul(plan.multiple, totalPrice)} 元
                    </div>
...
const mapStateToProps = (state) => {
  const { orderList, planList, issueList } = state
  return {
    orderList,
    planList,
    issueList
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    handleInitPlan: (issueCount, multiple) => {
      dispatch(initPlan(issueCount, multiple))
    },
